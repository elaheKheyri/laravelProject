...
<center>  
                ...
                <label style= " font-family: Bnazanin ; color: #8B0000 ; font-weight: bold "> :اطلاعیه ها </label>
				<br> </br>
				@if($post->text!= "nothing")
			    <p style = " font-family: Bnazanin ; color: #000000 " >{{$post->text}} </p>
			    @endif
						 <hr style="background-image: url(images/forhr.png) ; ">

 @auth
				@if (Auth::user()->role=='admin')
              <form action="/edit/notife" method="POST" enctype="multipart/form-data" style="font-size: 16px">
              @csrf
                <label style= " font-family: Bnazanin ; color: #8B0000 ; font-weight: bold" > : ویرایش یا اضافه کردن اطلاعیه جدید</label>
